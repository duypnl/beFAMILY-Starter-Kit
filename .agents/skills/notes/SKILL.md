---
name: Notes
description: Lưu tin nhắn nguyên văn vào sources/ kèm ảnh và chưng cất lên wiki.
...
1. Xác định thư mục brain hiện tại: `sources/`, `attachments/`, `wiki/` (chỉ chứa metadata), và các thư mục số beFAMILY (`10 FAITH/`, `20 LIFE/`, `30 WORK/`, `40 KNOWLEDGE/`, `50 OUTPUTS/`).
...
4. Đánh giá đáng-wiki (cùng tiêu chí ingest-source): có khái niệm / framework / nguyên lý /
   ...
   đồ -> KHÔNG đáng.
5. Nếu ĐÁNG: áp phép INGEST - viết/cập nhật file text nội dung gốc trực tiếp vào thư mục beFAMILY phù hợp, đủ 3 kỷ luật (mỗi claim cụ thể kèm `[[Nguồn]]`; phân biệt "(mục tiêu)"/"(thực tế)"; mâu thuẫn với trang cũ thì thêm `## Mâu thuẫn` + append `wiki/_open-questions.md`, KHÔNG ghi đè). Mỗi trang tự đủ ngữ cảnh (1-2 câu định vị đầu trang) + `aliases:` và metadata chuẩn. Đăng ký liên kết ánh xạ đầy đủ đường dẫn vào `wiki/index.md` (dạng `-[[thư-mục-số/Tên Trang]]: mô tả`). Append nhật ký vào `wiki/log.md`: `## [YYYY-MM-DD] notes | <tên note>` + đường dẫn file gốc + insight. Set source `status: processed`, `processed_at: <...>`, `wiki_links: [...]`.
